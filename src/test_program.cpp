...
int main()
{
	...
	while (1)
	{
		...
		if (connection.transferPacket(packet))
		{
			...
			packet = Packet(ID_L_SHOULDER_PITCH, Packet::WRITEW, P_GOAL_POSITION_L, 4096 - value);
			connection.transferPacket(packet);
		}
		...
		{
			int value = packet.getValue();
			packet = Packet(ID_L_SHOULDER_ROLL, Packet::WRITEW, P_GOAL_POSITION_L, 4096 - value);
			connection.transferPacket(packet);
		}
		...
		{
			int value = packet.getValue();
			packet = Packet(ID_L_ELBOW, Packet::WRITEW, P_GOAL_POSITION_L, 4096 - value);
			connection.transferPacket(packet);
		}
